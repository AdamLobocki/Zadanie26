...
public class TaskController {
    ...
    public String home(Model model, @RequestParam(required = false) TaskDone done) {
        ...
        if (done == null) {
            ...
            query = entityManager.createQuery("SELECT t from Task t  WHERE t.done = '" + done + "' order by t.endTime", Task.class);
        }
...
/*
    ...
    public String taskToEdit(Task task) {
        ...
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        entityManager.getTransaction().begin();
        TypedQuery<Task> query = entityManager.createQuery("UPDATE Task t SET t.done='ZROBIONE' WHERE  t.id= '" + task.getId() + "'  ", Task.class);
